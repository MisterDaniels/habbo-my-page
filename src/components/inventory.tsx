      ...
      <div id="inventory-modal" className="absolute flex flex-col mb-52 bg-white border border-black rounded-md max-w-[895px]">
        <div id="inventory-header" className="flex h-fit border-b border-black">
          <ul id="inventory-tabs" className="font-bold text-xs pt-1 px-2">
            <li className="px-3 text-black border-t border-x border-black rounded-t-md cursor-pointer">Inventory</li>
          </ul>
        ...
        <div id="inventory-body" className="flex px-3 py-5 text-xs">
            ...
            <div className="mt-1 bg-image-placeholder w-[150px] h-[306px]">
              <ul>
                { categories.map((category: ItemTypes, index: number) => {
                  return (
                    <li key={ index } className={ `px-3 text-black bg-city-dark capitalize ${ category.length - 1 !== index ? 'border-x border-t' : 'border' } border-black cursor-pointer [&.active]:bg-city ${ activeCategory === index ? 'active' : '' }` }
          ...
          <div id="inventory-items" className="ml-2 w-[516px]">
            <p className="text-black">Select an item by clicking it</p>
            <div className="mt-1">
              <ul className="flex flex-wrap gap-1">
                { items.map((item: ItemsList, index: number) => {
                  if (item.quantity > 0 && item.type === categories[activeCategory]) { 
                    return (
                      <li key={ index } className={ `relative w-[61px] h-[61px] bg-city-dark outline outline-black outline-1 [&.active]:outline-2 [&.active]:outline-tangerino [&.active]:bg-city hover:bg-city cursor-pointer ${ activeItem === index  ? 'active' : '' }` }
                        onClick={ () => { setActiveItem(index) } }>
                          <img
                            className="absolute object-none"
                            alt={ item.id.toString() }
                            src={ `/assets/${ categories[activeCategory] }s/${ (item.props as SingleImageComponentProps)?.category }/${ (item.props as SingleImageComponentProps)?.name }.${ (item.props as SingleImageComponentProps)?.extension ?? 'png' }` } />
                        { item.quantity > 1 && 
          ...
          <div id="inventory-preview" className="w-[150px] ml-5 mt-5 relative">
            <div className={ `w-[150px] h-[150px] ${ activeItem !== null ? 'border border-black' : ''}` }>
              { activeItem !== null && (items[activeItem]?.props as SingleImageComponentProps)?.name && 
                <img 
                  className={ `w-full h-full ${ items[activeItem].type === ItemTypes.Background ? 'object-fit'  : 'object-none' }` }
